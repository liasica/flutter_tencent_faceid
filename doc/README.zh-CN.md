...
5. 按脚本输出更新本 README 的 SDK 版本表、交付件校验表、zip 校验表、AAR 校验值、版本日志摘要、`tencent-sdk-changelogs/` 内的原始更新日志与 `CHANGELOG.md`。
...
以下内容根据本次腾讯交付件内的更新日志提炼，用于记录升级原因和回归重点。各 SDK 的完整原始更新日志（已转为 UTF-8）随仓库存放在 [tencent-sdk-changelogs/](tencent-sdk-changelogs/)，升级 SDK 时同步替换。
...
- 仓库根 `README.md` 与 `CHANGELOG.md` 改为英文（pub.dev 惯例检查），完整中文文档移至 `doc/README.zh-CN.md` 并保持为主要参考。
- 包描述改为英文，`bin/package_sdk.dart` 按 Dart 格式化规范整理。
...
- 附带腾讯各 SDK 的完整原始更新日志（`tencent-sdk-changelogs/`，已转 UTF-8）。
